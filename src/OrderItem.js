...
function OrderItem({ product, isOrdered }) {
  ...
  return (
    ...
    <div className="flex flex-col">
      <div className="flex items-center justify-between select-none">
        <div className="flex gap-2">
          <img src={`https://assets.csaposapp.hu/assets/images/${currentProduct?.imgUrl}`} alt="kép" className="h-10 aspect-square bg-white p-1 rounded-md" onError={(event) => event.target.src = img1}/>
          <div className="flex flex-col">
